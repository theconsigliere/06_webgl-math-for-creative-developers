# Three JS Workshops: Math for Creatives

[Three JS Workshops: Math for Creatives](https://threejs-workshops.com/workshop/math#1)

![Sin](sin.png)

## Sin

![Sine to Gerstner Waves](sine-to-gerstner-waves.png)

## Sine to Gerstner Waves

![Sine Smooth Transitions](sine-smooth-transitions.png)

## Sine Smooth Transitions

![Sine Texture Transitions](sine-texture-transitions.png)

## Sine Texture Transitions

![Sine to Loop Noise](sine-to-loop-noise.png)

## Sine to Loop Noise

![Derivatives in Graphics](derivatives-in-graphics.png)

## Derivatives in Graphics
